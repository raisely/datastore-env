...
class DatastoreEnvironment {
	...
	getProjectId() {
		let projectId = this.options.projectId || process.env.DATASTORE_PROJECT_ID || process.env.GOOGLE_CLOUD_PROJECT
			|| process.env.PROJECT || process.env.X_GOOGLE_GCLOUD_PROJECT || process.env.X_GOOGLE_GCP_PROJECT
...
function datastoreAvailable() {
	// Cache the result in production, but it will interfere with tests
	if (foundGcloud && (process.env.NODE_ENV !== 'test')) return true;

	// At least one of these tells us we are either in a gcloud environment
	// or we've got a datastore emulator available
	const projectVariables = ['GOOGLE_CLOUD_PROJECT', 'GOOGLE_GCLOUD_PROJECT',
		'X_GOOGLE_GCLOUD_PROJECT', 'GOOGLE_APPLICATION_CREDENTIALS',
		'DATASTORE_EMULATOR_HOST',
		'X_GOOGLE_GCP_PROJECT', 'GCLOUD_PROJECT', 'GCP_PROJECT',
		// Cloud functions 10
		'FUNCTION_SIGNATURE_TYPE'];

	for (let i = 0; i < projectVariables.length; i++) {
		const key = projectVariables[i];
		if (process.env[key]) {
			foundGcloud = true;
			return true;
		}
